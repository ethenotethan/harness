...
ARTIFACT_GUIDANCE = (
    ...
    "\n"
    "A `model` artifact may interleave narrative and operational views, for "
    "example [{\"type\": \"markdown\", \"text\": \"## Current sprint\"}, "
    "{\"type\": \"kanban\", \"entities\": [\"workstreams\"], "
    "\"column\": \"status\", \"columns\": [\"Todo\", \"Doing\", "
    "\"Done\"]}]. Kanban cards address parent-model entities as `set/key`; "
    "moving one writes its lane to the configured `column` field and needs "
    "no `actions` declaration.\n"
    "\n"
