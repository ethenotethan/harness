...
def _safe_load_unique(value: str) -> Any:
    return yaml.load(value, Loader=_UniqueKeyLoader)


@contextmanager
def _interprocess_write_lock(root: Path):
    """Serialize glossary compare-and-replace across gateway processes."""
    lock_path = root / f".{GLOSSARY_FILENAME}.lock"
...
def _nonempty_string(
    value: Any, field: str, *, max_length: int = MAX_TERM_LENGTH
) -> str:
...
def normalize_glossary(data: Any) -> dict[str, Any]:
    ...
    for index, raw in enumerate(raw_entries):
        ...
        if not isinstance(aliases_raw, list):
            raise GlossaryValidationError(
                f"{field}.aliases must be an array of strings"
            )
...
def load_glossary(wiki_root: str | os.PathLike[str]) -> dict[str, Any]:
    ...
    try:
        ...
        raise GlossaryValidationError(
            f"cannot read {GLOSSARY_FILENAME}: {exc}"
        ) from exc

    try:
        decoded = raw.decode("utf-8")
        parsed = _safe_load_unique(decoded)
...
def canonicalize_text(value: str, glossary: Mapping[str, Any]) -> str:
    ...
    return pattern.sub(
        lambda match: mapping.get(match.group(0).casefold(), match.group(0)),
        value,
    )
...
def update_glossary(
    ...
    *,
    ...
    with _WRITE_LOCK, _interprocess_write_lock(root):
        ...
        return {
            "enabled": True,
            **normalized,
            "revision": hashlib.sha256(rendered).hexdigest(),
        }
...
def load_configured_wikis() -> tuple[str | None, dict[str, str]]:
    ...
    try:
        parsed = _safe_load_unique(registry_path.read_text(encoding="utf-8"))
